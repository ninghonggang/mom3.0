...
func main() {
	...
	equipmentBomRepo := repository.NewEquipmentBomRepository(db)
	...
	productionCostRepo := repository.NewProductionCostRepository(db)
	...
	equipmentBomSvc := service.NewEquipmentBomService(equipmentBomRepo)
	...
	qualityCertificateRepo := repository.NewQualityCertificateRepository(db)
	qualityCertificateSvc := service.NewQualityCertificateService(qualityCertificateRepo)
	...
	productionCostSvc := service.NewProductionCostService(productionCostRepo, productionRepo)
...
aiSvc := service.NewAIService(aiConfigRepo, aiConversationRepo, aiMessageRepo)
	...
	vmiRepo := repository.NewVmiRepository(db)
	vmiSvc := service.NewVmiService(vmiRepo)
	vmiHandler := wms.NewVmiHandler(vmiSvc)
	...
	equipmentBomHandler := equipment.NewEquipmentBomHandler(equipmentBomSvc)
	...
	qualityCertificateHandler := quality.NewQualityCertificateHandler(qualityCertificateSvc)
	...
	costHandler := production.NewCostHandler(productionCostSvc)
	noticeHandler := system.NewNoticeHandler(noticeSvc)
	factoryHandler := system.NewFactoryHandler(factorySvc)
	...
	mobileJobReportRepo := repository.NewMobileJobReportRepository(db)
	mobileJobReportSvc := service.NewMobileJobReportService(mobileJobReportRepo)
	mobileJobReportHandler := mes.NewMobileJobReportHandler(mobileJobReportSvc)
	eamRepairJobHandler = eam.NewEamRepairJobHandler(eamRepairJobSvc)
	...
	idocRepo := repository.NewIdocRepository(db)
	idocSvc := service.NewIdocService(idocRepo)
	idocHandler := integration.NewIdocHandler(idocSvc)
	...
	customerCreditRepo := repository.NewCustomerCreditRepository(db)
	customerCreditSvc := service.NewCustomerCreditService(customerCreditRepo)
	customerCreditHandler := scp.NewCustomerCreditHandler(customerCreditSvc)
