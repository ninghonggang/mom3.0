...
-- 补 5 个父菜单(menu_data.sql 漏了)
INSERT INTO sys_menu (tenant_id, menu_name, menu_type, path, component, icon, sort, parent_id, status)
SELECT v.tenant_id, v.menu_name, v.menu_type, v.path, v.component, v.icon, v.sort, 0, 1
FROM (VALUES
    (1, '财务结算', 'M', '/fin', NULL, 'Money', 13),
    (1, '报表中心', 'M', '/report', NULL, 'PieChart', 14),
    (1, 'MES作业', 'M', '/mes', NULL, 'SetUp', 15),
    (1, '系统集成', 'M', '/integration', NULL, 'Connection', 16),
    (1, 'AGV调度', 'M', '/agv', NULL, 'VideoCamera', 17)
) AS v(tenant_id, menu_name, menu_type, path, component, icon, sort)
WHERE NOT EXISTS (SELECT 1 FROM sys_menu WHERE path = v.path AND tenant_id = 1);

-- 重跑 menu_data.sql 的 /fin 子菜单(LIMIT 1 防多行)
INSERT INTO sys_menu (tenant_id, menu_name, menu_type, path, component, icon, sort, parent_id, status)
SELECT 1, name.val, 'C', name.path, name.comp, name.icon, name.sort,
       (SELECT id FROM sys_menu WHERE path = '/fin' AND tenant_id = 1 LIMIT 1), 1
FROM (VALUES
  ('付款申请', '/fin/payment-request', 'fin/PaymentRequestList.vue', 'Money', 1),
  ('采购结算', '/fin/purchase-settlement', 'fin/PurchaseSettlementList.vue', 'ShoppingCart', 2),
  ('销售结算', '/fin/sales-settlement', 'fin/SalesSettlementList.vue', 'Sell', 3)
...
INSERT INTO sys_menu (tenant_id, menu_name, menu_type, path, component, icon, sort, parent_id, status)
SELECT 1, name.val, 'C', name.path, name.comp, name.icon, name.sort,
       (SELECT id FROM sys_menu WHERE path = '/report' AND tenant_id = 1 LIMIT 1), 1
FROM (VALUES
  ('生产日报', '/report/production-daily', 'report/ProductionDailyReport.vue', 'DataLine', 1),
  ('质量周报', '/report/quality-weekly', 'report/QualityWeeklyReport.vue', 'DataAnalysis', 2),
  ('OEE报表', '/report/oee', 'report/OeeReport.vue', 'TrendCharts', 3),
  ('交付分析', '/report/delivery', 'report/DeliveryReport.vue', 'TrendCharts', 4),
  ('Andon报表', '/report/andon', 'report/AndonReport.vue', 'DataAnalysis', 5)
) AS name(val, path, comp, icon, sort)
WHERE NOT EXISTS (SELECT 1 FROM sys_menu WHERE path = name.path AND tenant_id = 1);
...
INSERT INTO sys_menu (tenant_id, menu_name, menu_type, path, component, icon, sort, parent_id, status)
SELECT 1, name.val, 'C', name.path, name.comp, name.icon, name.sort,
       (SELECT id FROM sys_menu WHERE path = '/mes' AND tenant_id = 1 LIMIT 1), 1
FROM (VALUES
  ('班组', '/mes/team', 'mes/TeamList.vue', 'UserFilled', 1),
  ('工艺路线', '/mes/process-routes', 'mes/ProcessRouteList.vue', 'Connection', 2),
  ('离线作业', '/mes/offline', 'mes/OfflineJobList.vue', 'Document', 3),
  ('人员技能', '/mes/person-skill', 'mes/PersonSkillList.vue', 'Medal', 4),
  ('移动报工', '/mes/mobile-job-report', 'mes/MobileJobReportList.vue', 'Cellphone', 5)
) AS name(val, path, comp, icon, sort)
WHERE NOT EXISTS (SELECT 1 FROM sys_menu WHERE path = name.path AND tenant_id = 1);
...
INSERT INTO sys_menu (tenant_id, menu_name, menu_type, path, component, icon, sort, parent_id, status)
SELECT 1, name.val, 'C', name.path, name.comp, name.icon, name.sort,
       (SELECT id FROM sys_menu WHERE path = '/integration' AND tenant_id = 1 LIMIT 1), 1
FROM (VALUES
  ('接口配置', '/integration/interface-config', 'integration/InterfaceConfigList.vue', 'SetUp', 1),
  ('执行日志', '/integration/execution-log', 'integration/ExecutionLogList.vue', 'Document', 2),
  ('IDOC管理', '/integration/idoc', 'integration/IdocList.vue', 'Connection', 3)
) AS name(val, path, comp, icon, sort)
WHERE NOT EXISTS (SELECT 1 FROM sys_menu WHERE path = name.path AND tenant_id = 1);
...
INSERT INTO sys_menu (tenant_id, menu_name, menu_type, path, component, icon, sort, parent_id, status)
SELECT 1, name.val, 'C', name.path, name.comp, name.icon, name.sort,
       (SELECT id FROM sys_menu WHERE path = '/agv' AND tenant_id = 1 LIMIT 1), 1
FROM (VALUES
  ('AGV任务', '/agv/task', 'agv/AGVTaskList.vue', 'VideoPlay', 1),
  ('AGV设备', '/agv/device', 'agv/AGVDeviceList.vue', 'Van', 2),
  ('AGV站点', '/agv/location', 'agv/AGVLocationList.vue', 'Location', 3)
